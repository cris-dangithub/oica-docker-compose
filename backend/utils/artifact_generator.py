...
def generar_pdf_con_link_correcto(
    ...
    total_barra_m = resultados_df['barra_origen_longitud'].sum()
    total_desperdicio = resultados_df['desperdicio_m'].sum()
    # Eficiencia en metros (mismas unidades en numerador y denominador)
    eficiencia = ((total_barra_m - total_desperdicio) / total_barra_m * 100) if total_barra_m > 0 else 0
...
def generar_imagen_grafica(
    ...
    # Limitar a MAX_BARRAS_GRAFICA para evitar OOM en datasets grandes
    MAX_BARRAS_GRAFICA = 300
    es_muestra = len(barras) > MAX_BARRAS_GRAFICA
    if es_muestra:
        barras = barras[:MAX_BARRAS_GRAFICA]

    # Crear gráfica (altura proporcional al número de barras, máx razonable)
    alto_fig = max(7, min(len(barras) * 0.5, 150))
    fig, ax = plt.subplots(figsize=(16, alto_fig))
    ...
    titulo = 'Visualización de cortes por barra'
    if es_muestra:
        titulo += f' (muestra: primeras {MAX_BARRAS_GRAFICA} barras)'
    ax.set_title(titulo, fontsize=14, fontweight='bold')
    ax.set_yticks([])

    plt.tight_layout()

    # Guardar imagen (DPI reducido para datasets grandes)
    dpi_salida = 100 if es_muestra else 200
    plt.savefig(
        image_path,
        format='png',
        bbox_inches='tight',
