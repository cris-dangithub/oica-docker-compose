...
FILESTORE_DIR = os.path.abspath(os.environ.get('UPLOAD_PATH', os.path.join(os.path.dirname(__file__), 'data', 'filestore')))
...
def list_files():
    ...
    # Filtro por perfil — el perfil vive en ProcessingResult, no en UploadedFile
...
def reprocess_file(file_id):
    ...
    # Obtener perfil actual desde el último resultado procesado
...
def get_task_status(task_id):
    ...
    """
    try:
        # Un reinicio vacía los resultados de Redis; conservar el error en BD
        # permite que la pantalla de carga deje de esperar una tarea perdida.
        match = re.match(r'^(?:process|reprocess)_(\d+)(?:_|$)', task_id)
        if match:
            record = db.session.get(UploadedFile, int(match.group(1)))
            if record and record.processing_status == 'error_processing' and (record.status_details or '').startswith('Procesamiento interrumpido'):
                return jsonify({'task_id': task_id, 'state': 'FAILURE', 'progress': 0,
                                'message': record.status_details, 'error': True}), 200
